...
export default async function StudioPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  if(slug!==undefined){const work=typeof slug==="string"?publicWork(slug):undefined;if(!work)notFound();return <StudioGate key={`${work.slug}:${palette}`} initialDesign={remixWork(work,palette)} sourceSlug={work.slug}/>;}
