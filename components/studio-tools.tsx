...
import { Hand, Pencil, PaintBucket, Pipette, Replace, Scan, Move, Copy, ClipboardPaste, FlipHorizontal2, FlipVertical2, Repeat2, X, Palette, Highlighter, Undo2, Redo2, Ellipsis } from "lucide-react";
...
export function ToolButton({label,children,active,disabled,onClick,caption}:{label:string;children:React.ReactNode;active?:boolean;disabled?:boolean;onClick:()=>void;caption?:string}){
  return <Tooltip><TooltipTrigger asChild><Button variant="ghost" size={caption?"sm":"icon"} className={"editor-tool "+(active?"is-active ":"")+(caption?"labelled-tool":"")} aria-label={label} aria-pressed={active} disabled={disabled} onClick={onClick}>{children}{caption&&<span>{caption}</span>}</Button></TooltipTrigger><TooltipContent>{label}</TooltipContent></Tooltip>;
}
const tools=[{id:"brush",label:"Brush",caption:"Draw",icon:Pencil},{id:"fill",label:"Fill area",caption:"Fill",icon:PaintBucket},{id:"pick",label:"Pick colour",caption:"Pick",icon:Pipette},{id:"select",label:"Select area",caption:"Select",icon:Scan},{id:"pan",label:"Move chart",caption:"Pan",icon:Hand}];
...
export default function StudioTools(p:Props){
  ...
  const selectionActions=<>
    <ToolButton caption="Move" label="Move selection" active={p.tool==="move"} disabled={!p.selection} onClick={()=>{p.onTool("move");setMoreOpen(false);}}><Move/></ToolButton>
  ...
  return <div className={"studio-tools "+(p.compact?"compact-tools":"desktop-tools")} role="toolbar" aria-label={p.compact?"Quick editing tools":"Chart editing tools"}>
    ...
    <div className="tool-scroll">{tools.map(t=><ToolButton key={t.id} label={t.label} caption={t.caption} active={p.tool===t.id} onClick={()=>p.onTool(t.id)}><t.icon/></ToolButton>)}</div>
    <Popover open={moreOpen} onOpenChange={setMoreOpen}><PopoverTrigger asChild><Button variant="ghost" size="sm" className="more-tools-trigger" aria-label="More editing tools"><Ellipsis/><span>More</span></Button></PopoverTrigger><PopoverContent className="selection-popover" side={p.compact?"top":"bottom"} align="end"><b>More tools</b><div><ToolButton caption="Replace colour" label="Replace colour" active={p.tool==="replace"} onClick={()=>{p.onTool("replace");setMoreOpen(false);}}><Replace/></ToolButton><ToolButton caption="Highlight" label="Highlight selected colour" active={p.highlight} onClick={p.onHighlight}><Highlighter/></ToolButton></div><p>{p.selection?`${p.selection.rows} × ${p.selection.cols} selection`:"Select an area to copy, move, mirror or repeat it."}</p><div>{selectionActions}</div></PopoverContent></Popover>
    <div className="compact-history"><ToolButton label="Undo" disabled={!p.canUndo} onClick={p.onUndo}><Undo2/></ToolButton><ToolButton label="Redo" disabled={!p.canRedo} onClick={p.onRedo}><Redo2/></ToolButton></div>
  </div>;
}
