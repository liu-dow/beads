...
import type { ColorwayId } from "@/lib/portfolio";
...
const BraceletScene = lazy(() => import("./bracelet-scene").then(module => ({ default: module.BraceletScene })));
class SceneLoadBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
 state = { failed: false };
 static getDerivedStateFromError() { return { failed: true }; }
 render() { return this.state.failed ? <div className="scene-load-message" role="alert">The 3D preview could not load. You can keep designing in the 2D chart and try 3D again later.</div> : this.props.children; }
}
...
export default function Studio({initialDesign,initialSourceSlug}:{initialDesign?:Design;initialSourceSlug?:string}){
 ...
 useEffect(()=>{if(!opened.current){opened.current=true;trackConversion("studio_opened",{design:initialSourceSlug});}},[initialSourceSlug]);
 ...
 const [section,setSection]=useState("studio"),[mode,setMode]=useState("2d");
 ...
 const [dirty,setDirty]=useState(false),[saving,setSaving]=useState(false),[exporting,setExporting]=useState(false),[exportOpen,setExportOpen]=useState(false),[exportType,setExportType]=useState("png"),[transparent,setTransparent]=useState(false),[imageView,setImageView]=useState("2d");
 ...
 const [originalPalette,setOriginalPalette]=useState(design.palette);
 const skipLeaveGuard=useRef(false);
 const draft=useGuestDraft({enabled:isGuest,design,dirty,sourceSlug});
 const recoverable=entry==="remix"?draft.drafts.find(item=>item.sourceSlug===sourceSlug):draft.drafts[0];
 ...
 const quickPalette=starterPaletteId(design,originalPalette);
 ...
 const change=useCallback((fn:(d:Design)=>Design,history=true)=>{const next=fn(current.current);if(next===current.current)return;if(!firstEdit.current){firstEdit.current=true;trackConversion("first_edit",{design:sourceSlug});}setHasEdited(true);if(history){undoStack.current=[...undoStack.current.slice(-49),current.current];redoStack.current=[];setCanUndo(true);setCanRedo(false);}setDesign(next);setDirty(true);},[setDesign,sourceSlug]);
 ...
 useEffect(()=>{const f=(e:BeforeUnloadEvent)=>{if(!skipLeaveGuard.current&&(saving||(dirty&&(!isGuest||!flushDraft())))){e.preventDefault();e.returnValue="";}};window.addEventListener("beforeunload",f);return()=>window.removeEventListener("beforeunload",f);},[dirty,saving,isGuest,flushDraft]);
 useEffect(()=>{const timer=window.setTimeout(()=>{try{if(localStorage.getItem("bead-atelier:studio:intro-dismissed")==="1")setStarterVisible(false);}catch{/* The guide still works without persistent preferences. */}},0);return()=>window.clearTimeout(timer);},[]);
 ...
 const load=(work:WorkToOpen)=>{const d=work.design;draft.beginWorkspace(work.draft);setSourceSlug(work.draft?.sourceSlug);setDesign(d);setDirty(work.draft?.hasChanges??false);setEntry(work.entry);setHasEdited(!!work.draft?.hasChanges);firstEdit.current=false;setOriginalPalette(d.palette);undoStack.current=[];redoStack.current=[];setCanUndo(false);setCanRedo(false);setSelection(null);setClipboard(null);setSelected(0);setActiveCell(-1);setTool("brush");setMode("2d");setSection("studio");setPending(null);if(work.entry==="blank")setStarterVisible(true);window.history.replaceState(window.history.state,"","/studio");};
 ...
 const chooseQuickPalette=(id:ColorwayId)=>{if(id!==quickPalette)change(d=>applyStarterPalette(d,originalPalette,id));};
 ...
 const exportDesign=async()=>{
   ...
   try{
     ...
     trackConversion("design_exported",{design:sourceSlug,format:exportType});setExportOpen(false);
 ...
 return <TooltipProvider delayDuration={250}><div className={"atelier design-workspace "+(immersive?"immersive":"")}>
   ...
   <div style={{display:section==="studio"?"block":"none"}}>
     <section className="project-bar">
       ...
       <div className="project-actions"><Button variant="outline" className="subtle-btn" onClick={newDesign}><Plus/>New</Button><Button className="primary-button save-primary" onClick={requestSave} disabled={saving}><Save/>{saving?"Saving…":"Save work"}</Button><Button variant="outline" onClick={()=>{setImageView(ready&&scene.current?"3d":"2d");setExportOpen(true);}}><Download/>Export work</Button></div>
     </section>
         ...
         <div className={"viewport mode-"+mode} style={{display:mode==="making"?"none":undefined}}>
          <ResizablePanelGroup orientation={isMobile?"vertical":"horizontal"} className="split-panels" id="studio-views">
           ...
           <ResizablePanel id="scene" className="scene-panel" defaultSize={isMobile?"50%":"40%"} minSize="25%" style={{display:mode==="2d"?"none":undefined}}><div className={"three-stage "+(background==="#242d30"?"dark-stage":"")}>
             {section==="studio"&&(mode==="3d"||mode==="split")&&<SceneLoadBoundary><Suspense fallback={<div className="scene-load-message" role="status"><LoaderCircle className="animate-spin" size={20}/>Loading the 3D preview…</div>}><BraceletScene ref={scene} design={design} shape={shape} light={light} background={background} rotate={rotate} editing={editing&&!["pan","select","move","paste"].includes(tool)} onPaint={i=>onPaint(i,true)} onReady={setReady} activeCell={activeCell} onHover={setActiveCell} highlightColor={highlight?selectedIndex:undefined} /></Suspense></SceneLoadBoundary>}
