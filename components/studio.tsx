...
import { publicWork } from "@/lib/portfolio";
...
function PatternThumb({design}:{design:Design}){
 ...
 return <canvas ref={ref} className="collection-thumb" aria-label={design.title+"Pattern"}/>;
}
export default function Studio({initialDesign}:{initialDesign?:Design}){
 ...
 const [dirty,setDirty]=useState(!!initialDesign),[saving,setSaving]=useState(false),[exporting,setExporting]=useState(false),[exportOpen,setExportOpen]=useState(false),[exportType,setExportType]=useState("png"),[transparent,setTransparent]=useState(false),[imageView,setImageView]=useState("3d");
 ...
 const requestLoad=(d:Design)=>{if(saving){toast.info("Your design is being saved. Please wait before switching.");return;}if(dirty)setPending(d);else load(d);};
 const newDesign=()=>{const d=createDesign("coast");d.title="Untitled design";d.description="";d.author=design.author;d.cells.fill(0);requestLoad(d);};
 ...
 const exportDesign=async()=>{
   ...
   try{
     ...
     if(isGuest){recordGuestExport(exportType as "png"|"pdf");toast.success(exportType==="png"?"Preview image created":"PDF making chart created");}
 ...
 return <TooltipProvider delayDuration={250}><div className={"atelier "+(immersive?"immersive":"")}>
   <header className="app-header">
     <Link href="/" className="brand" onClick={e=>{if(dirty||saving){e.preventDefault();toast.info("Save your design before returning home.");}}} aria-label="Back to Bead Atelier home"><span className="brand-mark"><Gem size={25} strokeWidth={1.25}/></span><span><b>Bead Atelier<span className="brand-dot">.</span></b><small>THE DESIGN STUDIO</small></span></Link>
     ...
     <div className="header-end"><Link href="/portfolio" className="studio-gallery-link" onClick={event=>{if(dirty||saving){event.preventDefault();toast.info("Save your current work before opening the gallery.");}}}>Gallery <ArrowUpRight size={14}/></Link><span className="studio-tag">{isGuest?"Guest · Saved in this browser":"THE MAKER'S STUDIO"}</span><IconButton label="Guide" onClick={()=>setHelp(true)}><CircleHelp/></IconButton><button className="avatar" aria-label={isGuest?"Guest mode":"Account details"} onClick={()=>setAccountOpen(true)}><UserRound size={19}/></button></div>
   </header>
   <main>
   <div style={{display:section==="studio"?"block":"none"}}>
     <section className="project-bar">
       <div><div className="eyebrow">Your ideas, bead by bead <span>/</span> PEYOTE STUDIO</div><div className="project-title"><h1>{design.title}</h1><IconButton label="Edit work details" onClick={openMeta}><Pencil size={15}/></IconButton><span className="draft-label">{dirty?"Unsaved changes":design.id?"Saved":"New design"}</span></div><p className="byline">Design / {design.author}<span>·</span>beaded bracelet</p></div>
       <div className="project-actions"><Button variant="outline" className="subtle-btn" onClick={newDesign}><Plus/>New</Button><Button variant="outline" onClick={save} disabled={saving}><Save/>{saving?"Saving…":"Save work"}</Button><Button className="primary-button" onClick={()=>setExportOpen(true)}><Download/>Export work<ChevronRight size={15}/></Button></div>
     </section>
     <div className={"studio-layout "+(leftCollapsed?"left-collapsed ":"")+(rightCollapsed?"right-collapsed":"")}>
       <aside className="left-panel panel">
         <div className="panel-heading"><span>Making materials</span><span className="count-badge">{design.palette.length} colours</span></div>
         <div className="section-label"><span>Cylinder beads</span><span className="muted-label">{design.size.toFixed(1)} mm</span></div>
         ...
         <div className="panel-bottom"><Gem size={17}/><span>Room for every handmade idea.</span></div>
       </aside>
       <section className="work-surface">
         <div className="viewport-toolbar">
           <Tabs value={mode} onValueChange={v=>{setMode(v);setActiveCell(-1);}}><TabsList className="view-tabs"><TabsTrigger value="2d"><Grid2X2/>2D chart</TabsTrigger><TabsTrigger value="3d"><Box/>3D simulation</TabsTrigger><TabsTrigger value="split"><Columns2/>Split view</TabsTrigger><TabsTrigger value="making"><ListChecks/>Making</TabsTrigger></TabsList></Tabs>
           <div className="tool-cluster"><IconButton label={isMobile||immersive?"Material settings":leftCollapsed?"Expand materials panel":"Collapse materials panel"} active={leftCollapsed} onClick={()=>{if(isMobile||immersive)setDrawer("materials");else setLeftCollapsed(v=>!v);}}><PanelLeftClose/></IconButton><IconButton label="Open bead library" onClick={()=>setLibraryOpen(true)}><BookOpen/></IconButton><IconButton label={isMobile||immersive?"Size & materials":rightCollapsed?"Expand settings panel":"Collapse settings panel"} active={rightCollapsed} onClick={()=>{if(isMobile||immersive)setDrawer("settings");else setRightCollapsed(v=>!v);}}><PanelRightClose/></IconButton><span className="toolbar-history"><IconButton label="Undo (Ctrl+Z)" onClick={undo} disabled={!canUndo||mode==="making"}><Undo2/></IconButton><IconButton label="Redo (Ctrl+Shift+Z)" onClick={redo} disabled={!canRedo||mode==="making"}><Redo2/></IconButton></span><IconButton label={immersive?"Exit fullscreen studio":"Fullscreen studio"} active={immersive} onClick={()=>setImmersive(v=>!v)}>{immersive?<X/>:<Maximize2/>}</IconButton></div>
         </div>
         ...
         <div className={"viewport mode-"+mode} style={{display:mode==="making"?"none":undefined}}>
          <ResizablePanelGroup orientation={isMobile?"vertical":"horizontal"} className="split-panels" id="studio-views">
           ...
           <ResizablePanel id="pattern" className="pattern-panel" defaultSize="50%" minSize="25%" style={{display:mode==="3d"?"none":undefined}}><div className="two-stage">
             <div className="two-toolbar"><span><Grid2X2 size={15}/> beadwork chart</span><div><IconButton label="Zoom out chart" onClick={()=>setZoom(z=>Math.max(.5,z-.25))}><ZoomOut/></IconButton><span>{Math.round(zoom*100)}%</span><IconButton label="Zoom in chart" onClick={()=>setZoom(z=>Math.min(2,z+.25))}><ZoomIn/></IconButton></div></div>
       ...
       <aside className="right-panel panel">
         <div className="panel-heading"><span>Design settings</span><SlidersHorizontal size={17}/></div>
         ...
         <div className="settings-section material-section"><div className="section-label"><span>Material quantities</span><span className="small-tag">{counts.length} colours</span></div>
           ...
           <p className="microcopy">Reserve includes {design.fit?.allowance??5}% allowance.Purchase qty. excludes stock.</p>
         </div>
   ...
   <Sheet open={drawer!==null} onOpenChange={v=>{if(!v)setDrawer(null);}}><SheetContent side={isMobile?"bottom":"right"} className="workspace-sheet"><SheetHeader><SheetTitle>{drawer==="materials"?"Material settings":"Size & materials"}</SheetTitle><SheetDescription>{design.title}</SheetDescription></SheetHeader><div className="workspace-sheet-body">{drawer==="materials"?<MaterialsEditor design={design} selected={selectedIndex} onSelect={setSelected} change={change} onPaletteStructure={paletteStructure} onOpenLibrary={()=>setLibraryOpen(true)}/>:<><SizeSettings design={design} change={change} onResize={clearSelection}/><div className="section-label">Material quantities / beads</div><MaterialList design={design} onSelect={i=>{setSelected(i);setDrawer("materials");}}/></>}</div></SheetContent></Sheet>
   ...
   <Dialog open={!!pending} onOpenChange={v=>{if(!v)setPending(null);}}><DialogContent><DialogHeader><DialogTitle>Keep this idea before switching?</DialogTitle><DialogDescription>This design has unsaved changes. Return to the studio to save, or open the next design.</DialogDescription></DialogHeader><div className="dialog-actions"><Button variant="outline" onClick={()=>setPending(null)}>Back to save</Button><Button onClick={()=>{if(pending)load(pending);}}>Open next design</Button></div></DialogContent></Dialog>
   <Dialog open={help} onOpenChange={setHelp}><DialogContent><DialogHeader><DialogTitle>Start with a single bead</DialogTitle><DialogDescription>Take an idea from a flat chart to a three-dimensional design.</DialogDescription></DialogHeader><ol className="help-list"><li><b>Choose a pattern and palette</b><p>Start with a blank chart or continue a saved design in My work.</p></li><li><b>Edit bead by bead</b><p>Drag on the 2D chart to paint. Use the brush, fill, eyedropper, or colour replacement tool. Press Ctrl / ⌘ + Z to undo.</p></li><li><b>Explore material finishes</b><p>Drag to rotate and scroll to zoom. Try different finishes, lighting, and loop or unfolded views. Choose Paint to change individual beads.</p></li><li><b>Share and make</b><p>Add your author details, save the design, and export a high-resolution PNG or a PDF with numbered charts and a material list.</p></li></ol></DialogContent></Dialog>
